...
const toolImplementations = {
    read_logs: async ({ service, limit = 20, startTime }) => {
        const query = { service }
        if (startTime) {
        ...
        const logs = await LogEntry.find(query).sort({ timestamp: -1 }).limit(limit)
        return logs.map(l => ({
            level: l.level,
            message: l.message,
    ...
    get_metrics: async ({ target, durationMinutes = 15 }) => {
        ...
        const results = await MetricSnapshot.find(query).sort({ timestamp: -1 }).limit(10)
    ...
    check_deployments: async ({ service, limit = 5 }) => {
    ...
    query_database: async ({ collection, filter }) => {
        ...
        return await db.collection(collection).find(filter).limit(10).toArray()
    },
    ...
    send_alert: async ({ severity, message }) => {
        ...
        return { logged: true, severity, message }
    }
}
